...
namespace BerryCore.SOA.API.Handlers
{
    /// <summary>
    /// 请求频率限制
    /// </summary>
    public sealed class RequestFrequencyLimitHandlers : DelegatingHandler
    {
        ...
        {
            //Key，即IP地址
            string identifier = this.GetUserIdentifier(request);

            if (string.IsNullOrEmpty(identifier))
            {
                return this.CreateResponse(request, HttpStatusCode.Forbidden, "无法识别客户机.");
            }
            ...
            Task<HttpResponseMessage> response = entry.Requests > maxRequests ? this.CreateResponse(request, HttpStatusCode.Conflict, string.Format(_message, maxRequests)) : base.SendAsync(request, cancellationToken);
