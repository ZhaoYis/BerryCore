...
namespace BerryCore.SOA.API.Selector
{
    /// <summary>
    /// WebApi版本控制
    /// </summary>
    ...
    public class WebApiControllerSelector : IHttpControllerSelector
    {
        ...
        public HttpControllerDescriptor SelectController(HttpRequestMessage request)
        {
            ...
            if (routeData == null)
            {
                ...
                {
                    Status = GlobalErrorCodes.RecordNotExists,
                    Message = "系统找不到对应的路由器"
                };
                throw new HttpResponseException(request.CreateResponse(HttpStatusCode.NotFound, resultMsg));
            }
            ...
            else
            {
                ...
                if (!string.IsNullOrEmpty(version) && !key.Contains(version + "."))
                {
                ...
                else if (_duplicates.Contains(key))
                {
                    BaseJsonResult<string> resultMsg = new BaseJsonResult<string>
                    {
                        Status = GlobalErrorCodes.MultipleRecords,
                        Message = "找到了与此请求相匹配的多个控制器"
                    };
                    throw new HttpResponseException(request.CreateResponse(HttpStatusCode.InternalServerError, resultMsg));
                }
                else
                {
                    BaseJsonResult<string> resultMsg = new BaseJsonResult<string>
        ...
        private string GetVersionFromAcceptHeaderVersion(HttpRequestMessage request)
        {
            ...
            foreach (var mime in acceptHeader)
            {
                ...
                {
                    NameValueHeaderValue version = mime.Parameters.FirstOrDefault(v => v.Name.Equals(VersionKey, StringComparison.OrdinalIgnoreCase));
