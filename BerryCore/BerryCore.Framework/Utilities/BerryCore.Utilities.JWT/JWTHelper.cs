...
namespace BerryCore.Utilities.JWT
{
    ...
    public sealed class JWTHelper
    {
        ...
        static JWTHelper()
        {
            string path = string.Format("{0}SecretKey\\token.privateKey.key", AppDomain.CurrentDomain.BaseDirectory);
            if (File.Exists(path))
            {
        ...
        public static string GetToken(JWTPlayloadInfo playload)
        {
            ...
            playload.exp = DateTime.Now.AddHours(2).DateTime2TimeStamp().ToString();
