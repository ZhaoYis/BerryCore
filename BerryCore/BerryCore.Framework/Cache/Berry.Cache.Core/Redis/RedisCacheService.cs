...
namespace Berry.Cache.Core.Redis
{
    ...
    public class RedisCacheService : BaseCacheService, ICacheService
    {
        ...
        public void SetDefaultExpireTime(TimeSpan time)
        {
        ...
        public void SetDefaultCacheKeyPrefix(string prefix)
        {
        ...
        public void SetRedisDbNumber(int num)
        {
            if (num >= 0)
            {
                _redisHelper = new RedisHelper(num);
            }
        }
        ...
        /// </summary>
        protected override TimeSpan DefaultExpireTime
        {
            get
            {
        ...
        {
            return _redisHelper.KeyExists(this.GetCacheKey(key));
        }
        ...
        {
            return _redisHelper.StringSet(this.GetCacheKey(key), value, DefaultExpireTime);
        }
        ...
        {
            return _redisHelper.StringSet(key, value, expiresSliding ?? expiressAbsoulte ?? DefaultExpireTime);
        }
        ...
        public bool Add(string key, object value, TimeSpan? expiresIn, bool isSliding = false)
        {
            if (!isSliding)
            {
                return _redisHelper.StringSet(this.GetCacheKey(key), value, expiresIn);
            }
            else
            {
                if (this.Exists(key))
        ...
        {
            return _redisHelper.KeyDelete(this.GetCacheKey(key));
        }
        ...
        public void Remove(Func<string, bool> func)
        {
            ...
            foreach (string key in _redisHelper.GetKeys())
            {
                if (func.Invoke(key))
                {
                    keys.Add(key);
        ...
        {
            List<string> keys = _redisHelper.GetKeys();
            this.RemoveAll(keys);
        }
        ...
        {
            T value = _redisHelper.StringGet<T>(this.GetCacheKey(key));
            return value;
        }
        ...
        {
            object o = _redisHelper.StringGet(this.GetCacheKey(key));
            return o;
        }
        ...
        {
            if (_redisHelper.KeyExists(this.GetCacheKey(key)))
            {
                _redisHelper.KeyDelete(this.GetCacheKey(key));
                return this.Add(key, value);
            }
            else
            {
