...
namespace BerryCore.MQ.Base
{
    ...
    /// 基础消息模型
    /// </summary>
    public class BaseMqMessageEntity : IBaseMqMessage
    {
        /// <summary>
        /// 基础消息模型（默认初始化方式，可以自定义签名计算方式）
        /// </summary>
        /// <remarks>签名计算方式：MD5（{秒级时间戳}{数据包}{秒级时间戳}）</remarks>
        protected BaseMqMessageEntity()
        {
            this.CreateTime = this.GetTimeStamp(DateTime.Now);
        ...
        {
            string sign = this.Platform + this.ClientId + this.Command + this.CreateTime;
            return sign;
        }
