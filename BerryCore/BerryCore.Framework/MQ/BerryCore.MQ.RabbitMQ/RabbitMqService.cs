...
* 作    者 ：赵轶
* 创建时间 ：2019-05-30 10:35:50
* 更新时间 ：2019-05-30 10:35:50
* 版 本 号 ：V2.0.0.0
***********************************************************************
...
{
    /// <summary>
    /// 功能描述    ：RabbitMqService  
    /// 创 建 者    ：赵轶
    /// 创建日期    ：2019-05-30 10:35:50 
    /// 最后修改者  ：赵轶
    /// 最后修改日期：2019-05-30 10:35:50 
    /// </summary>
    public sealed class RabbitMqService : BaseLogger, IDisposable
    {
        ...
        /// <param name="type">交换机类型</param>
        /// <returns></returns>
        public bool Publish<T>(T command, string type = ExchangeTypeCode.Direct) where T : class, IBaseMqMessage
        {
            ...
            string body = command.TryToJson();
            string exchange = queueInfo.ExchangeName;
            string queue = queueInfo.QueueName;
            string routingKey = queueInfo.RoutingKey;
            bool isProperties = queueInfo.IsProperties;

            return this.Publish(exchange, queue, routingKey, body, type, isProperties);
        }
        ...
        /// <param name="isProperties">是否持久化</param>
        /// <returns></returns>
        private bool Publish(string exchange, string queue, string routingKey, string body, string type, bool isProperties = false)
        {
            bool res = false;
        ...
        /// <returns></returns>
        private bool PublishToDead<T>(string queue, string body, string type, Exception ex) where T : class
        {
            var queueInfo = this.GetRabbitMqAttribute<T>();
            if (queueInfo == null)
            {
            ...
            string deadLetterExchange = queueInfo.ExchangeName;
            string deadLetterRoutingKey = queueInfo.RoutingKey;
            ...
            return this.Publish(deadLetterExchange, deadLetterQueue, deadLetterRoutingKey, deadLetterBody.TryToJson(), type, isProperties);
        }
